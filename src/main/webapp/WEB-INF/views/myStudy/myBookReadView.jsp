	...
	<div class="serchpos">
		...
		<div class="serchbookDiv">
			<c:forEach var="bookRead" items="${bookReadList}">
				<div class="genre1_book_list" onmouseenter="bookCapOn(${bookRead.readBookNum})" onmouseleave="bookCapOff(${bookRead.readBookNum})">
					<a name="bookArticleNum" href="/textant/myBookReadDelete.text?pageNum=${pageNum}&readBookNum=${bookRead.readBookNum}">삭제</a><br>
					<a href="/textant/read.text?fileName=${bookRead.fileLocation}">
						<img src="displayFile.text?fileName=${bookRead.fileLocation}/OriginImg&pageNum=1&fileType=jpg" style="border: 1px solid #D8D8D8; width: 166.45px; height: 244px; position: absolute; z-index: 2; height: 242px;" class="book_img_size" id="bookimg" >

							<div style="width: 166.45px; height: 244px; position: relative; text-align: center; color: white; background: gray;z-index: 1; background-color: rgba(0, 0, 0, 0.5);" class="book_cap${bookRead.readBookNum}" id="book_cap${bookRead.readBookNum}">
<%-- 								<h5><c:out value="${bookWrite}"></c:out></h5> --%>
     								<p>${bookRead.bookName}</p>
									<br>
									<p>${bookRead.bookDesc}</p>
							</div> 
