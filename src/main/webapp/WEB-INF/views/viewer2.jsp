...
#coShow{margin-top:3%;}
#coShow table{width:100%; border-top:1px white dashed; }
...
function loadApp() {
	...
	$('.magazine-viewport').zoom({
		...
		max: function() { 
			...
			return largeMagazineWidth()/$('.sample-docs').width();
		...
		when: {
			...
			zoomIn: function () {
				...
				$('#slider-bar').hide();
				$('.made').hide();
				$('.sample-docs').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.sample-docs').addClass('animated').removeClass('zoom-in');
					resizeViewport();
				}, 0);
...
yepnope({
	...
	both: ['/textant/resources/css/docs1.css', '/textant/resources/js/docs.js',
		'/textant/resources/js/zoom.min.js'],
	complete: loadApp
});
//*******************
	...
	$.ajax({
		...
		success: function(data){
			 ...
			 $.each(data, function(index,item) {
				 ...
				 html='<table border="1" class="padding10 commentDelete'+commentNum+'">'+
					'<tr>'+
					'<td rowspan="4" width="30px">+ profilePicture+</td>'+
					'<td colspan="2" ><i class="font7">'+item.commentNum+'</i><br>'+item.conet+'</td>'+
				'</tr>'+
				'<tr>'+
